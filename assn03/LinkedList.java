...
public class LinkedList<T> {
    ...
    public void multiply(int factor) {
        if (factor == 0) {
            ...
            Node<T> cnode = head;
            Node<T> nextNode = cnode.getNext();
            int j = 0;
            while (j < size * factor && cnode != null) {
                nextNode = cnode.getNext();
                for (int i = 1; i < factor; i++) {
                    this.add(i + j, cnode.getValue());
                }
                j = j + factor;
                cnode = nextNode;
            }
...
//     * @param list2
    ...
    public void merge(LinkedList list2) {
        ...
        Node<T> next1 = head1.getNext();
        Node<T> next2 = head2.getNext();
        int j = 0;
        clear();
        while (head1 != null || head2 != null) {
            ...
            if (j % 2 == 0 || head2 == null) {
                this.add(j, head1.getValue());
                head1 = next1;
                j += 1;
            } else if (j % 2 != 0) {
                this.add(j, head2.getValue());
                head2 = next2;
                j += 1;
            }
        }
